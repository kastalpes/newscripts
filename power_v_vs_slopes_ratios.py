"""
Make velocity vs slopes ratios.
...
for axes in ['x','y','z']:
    ...
    for i in range(12):
      ...
      avg_v_list[i]=v
      avg_k_list[i]=kval
      slopefind=list(plfit(ds.lcent,v,fitrange)) #returns [slope,amp,res]
      ...
      #ax1.plot(xslope,yslope,label=r'$\alpha^{EE}$ %0.2f'%avg_slop
    xs=avg_slopes_list
    ys1=avg_ee_slopes
    ys2=avg_bb_slopes
    ys3=avg_ratio_list
    ...
    ax1.set_xlabel(r'V Slope')
    ax1.set_ylabel(r'ClEE Slope')
    ax1.set_title(r'V EE Parameter Space')
    ax1.legend(loc=0)
    fig1.savefig("%s/ee_V_slopes_parspace_%s.png"%(plot_dir,axes))
    ax2.plot(xs, ys2,'.')
    y2=[i*m2 for i in xs]
    yy2=[i+b2 for i in y2]
    ax2.plot(xs,yy2,'-',label=np.round(m2,decimals=3))
    ax2.set_xlabel(r'V Slope')
    ax2.set_ylabel(r'ClBB Slope')
    ax2.set_title(r'V BB Parameter Space')
    ax2.legend(loc=0)
    fig2.savefig("%s/bb_V_slopes_parspace_%s.png"%(plot_dir,axes))
    ax3.plot(xs, ys3,'.')
    y3=[i*m3 for i in xs]
    yy3=[i+b3 for i in y3]
    ax3.plot(xs,yy3,'-',label=np.round(m3,decimals=3))
    ax3.set_xlabel(r'V Slope')
    ax3.set_ylabel(r'EE/BB Ratio')
    ax3.set_title(r'V E/B Ratio Parameter Space')
    ax3.legend(loc=0)
    fig3.savefig("%s/V_ratio_parspace_%s.png"%(plot_dir,axes))
