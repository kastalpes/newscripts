...
#        frb_dir = "%s/%s"%(self.simulation.directory,p49_QU2EB.frbname)
        frb_dir = "%s"%self.simulation.directory
    ...
    def determine_fit_range(self):
        ...
        if self.bin_style == 'dx1':
            fitrange[0] = self.lcent[2]  #was 4
            fitrange[1]=  self.lcent[30] #was 10
        else:
...
class simulation_package():
    ...
    def make_spectra(self,frame):
        ...
        st.MakeVxSpectra(oober,frame)
        st.MakeVySpectra(oober,frame)
        st.MakeVzSpectra(oober,frame)
