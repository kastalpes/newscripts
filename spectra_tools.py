...
def MakeVxSpectra(oober,frame,density=0,debug=1):
    """density = 0,1,2 for V, \rho^1/2 V, \rho^1/3 V"""
    power=0
    rhohat = oober.fft(frame,'velocity_x',num_ghost_zones=ngz,debug=debug)
    power += (rhohat.conjugate()*rhohat)
    field_out='velocity_x'
    fname = shell_average(power,oober,frame,field_out,debug,mark_time)
    print(fname)

def MakeVySpectra(oober,frame,density=0,debug=1):
    """density = 0,1,2 for V, \rho^1/2 V, \rho^1/3 V"""
    power=0
    rhohat = oober.fft(frame,'velocity_y',num_ghost_zones=ngz,debug=debug)
    power += (rhohat.conjugate()*rhohat)
    field_out='velocity_y'
    fname = shell_average(power,oober,frame,field_out,debug,mark_time)
    print(fname)

def MakeVzSpectra(oober,frame,density=0,debug=1):
    """density = 0,1,2 for V, \rho^1/2 V, \rho^1/3 V"""
    power=0
    rhohat = oober.fft(frame,'velocity_z',num_ghost_zones=ngz,debug=debug)
    power += (rhohat.conjugate()*rhohat)
    field_out='velocity_z'
    fname = shell_average(power,oober,frame,field_out,debug,mark_time)
    print(fname)
